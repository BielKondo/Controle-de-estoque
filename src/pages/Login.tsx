...
import { Package2, Building2, Mail, Lock, Eye, EyeOff, ArrowRight, ShieldCheck, Cloud, CheckCircle2, Loader2 } from 'lucide-react';
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col bg-background-light dark:bg-background-dark overflow-x-hidden font-display">
      ...
      <header className="flex items-center p-6 justify-between max-w-[480px] mx-auto w-full">
        ...
        <div className="text-slate-500 dark:text-slate-400 text-sm font-medium">v2.4.0</div>
      </header>
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 pb-12 z-10">
        <div className="w-full max-w-[440px] space-y-8">
          ...
          {showReset ? (
            /* ── Reset / First access panel ── */
            <div className="space-y-6">
              <div className="text-center space-y-2">
                <h1 className="text-slate-900 dark:text-white text-2xl font-bold">{t.resetPasswordTitle}</h1>
              ...
              {resetSent ? (
                <div className="flex flex-col items-center gap-3 py-6 text-center">
                  ...
                  <p className="font-semibold text-emerald-600">{t.resetSent}</p>
                </div>
              ) : (
                <form onSubmit={handleReset} className="space-y-4">
                  {resetError && (
                    <div className="bg-red-50 dark:bg-red-900/30 text-red-600 text-sm p-3 rounded-xl text-center">
                      {resetError}
                    </div>
                  )}
            ...
            <>
              ...
              <form onSubmit={handleLogin} className="space-y-5">
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-slate-700 dark:text-slate-300 text-sm font-semibold px-1">{t.companyId}</label>
                  <div className="relative">
                ...
                <div className="flex items-center gap-2 px-1">
                  <input id="remember" type="checkbox" className="rounded border-slate-300 text-primary focus:ring-primary" />
              ...
              <div className="text-center pt-2">
                <p className="text-slate-500 dark:text-slate-400 text-sm">
                  {t.newPlatform}{' '}
                  <a href="#" className="text-primary font-bold hover:underline">{t.contactSupport}</a>
                </p>
              </div>
            </>
          )}
      ...
      <footer className="mt-auto w-full py-10 px-6 border-t border-slate-200 dark:border-slate-800 z-10">
        <div className="max-w-6xl mx-auto flex flex-col lg:flex-row justify-between items-center gap-6">
          ...
          <div className="text-slate-400 text-xs text-center">© 2024 StockFlow Systems Inc. All rights reserved.</div>
        </div>
      </footer>
